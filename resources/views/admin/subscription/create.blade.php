            ...
            <div class="p-6 bg-white shadow-xl sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('admin.subscriptions.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="subjects[]" id="subjects" multiple
                            ...
                            @foreach ($subjects as $subject)
                                <option value="{{ $subject->id }}" @if (collect(old('subjects'))->contains($subject->id)) selected @endif>
                                    {{ $subject->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <button type="submit"
                            class="px-4 py-2 font-semibold text-white bg-indigo-600 rounded hover:bg-indigo-700">Create</button>
                    </div>
                </form>
            </div>
        </div>
