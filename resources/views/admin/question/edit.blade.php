<x-app-layout>
  <div class="max-w-7xl p-10 mx-auto bg-white border border-gray-200 shadow-[0_8px_30px_rgba(55,55,55,0.1)] rounded-3xl"
    ...
    <form method="POST" action="{{ route('admin.questions.update', $question->id) }}" enctype="multipart/form-data" class="space-y-8">
      @csrf
      @method('PUT')
      ...
      <div>
        ...
        <input type="text" name="question_data[education_type]" value="{{ ucfirst($question->education_type) }}" readonly
      ...
      <div>
        ...
        <textarea name="question_data[content]" rows="4" required
                  class="w-full p-4 text-lg text-black border border-gray-500 rounded-xl">{{ old('question_data.content', $question->content) }}</textarea>
      </div>
          ...
          @foreach ($question->options as $index => $option)
            <div class="flex items-center gap-4 p-3 mb-4 border-2 bg-gray-50 rounded-xl">
              <input type="radio" name="question_data[correct_option]" value="{{ $index }}"
                     {{ old('question_data.correct_option', $option->is_correct) == $index ? 'checked' : '' }} required
                     class="w-6 h-6 accent-gray-500" />
              <input type="text" name="question_data[options][]" value="{{ old('question_data.options.' . $index, $option->value) }}"
        ...
        <div>
          ...
          @foreach ($blanks as $i => $blank)
            <div class="p-4 mb-6 border border-gray-300 rounded-lg bg-gray-50">
              ...
              <input type="hidden" name="question_data[blanks][{{ $i }}][blank_number]" value="{{ $blank['blank_number'] ?? $i + 1 }}">
              ...
              <select name="question_data[blanks][{{ $i }}][answer]" required
                ...
                <option value="" disabled {{ empty($blank['answer']) ? 'selected' : '' }}>Select correct answer</option>
                @foreach ($blank['options'] ?? [] as $option)
                  <option value="{{ $option }}" {{ ($blank['answer'] ?? '') === $option ? 'selected' : '' }}>{{ $option }}</option>
                @endforeach
              </select>
              <label class="block mb-1 font-semibold text-gray-700">Options</label>
              @foreach ($blank['options'] ?? [] as $optIndex => $option)
                <input type="text" name="question_data[blanks][{{ $i }}][options][]" value="{{ $option }}" required
      ...
      @elseif ($question->type === 'true_false')
        <div>
          ...
          <select name="question_data[true_false_answer]" required
            ...
            <option value="True" {{ old('question_data.true_false_answer', $question->metadata['answer']['choice'] ?? '') == 'True' ? 'selected' : '' }}>True</option>
            <option value="False" {{ old('question_data.true_false_answer', $question->metadata['answer']['choice'] ?? '') == 'False' ? 'selected' : '' }}>False</option>
          </select>
        </div>
              ...
              <div class="mb-4">
                ...
                @if (($pair['left']['match_type'] ?? 'text') === 'image')
                  <div class="mb-2">
                    ...
                    <input type="hidden" name="question_data[options][{{ $i }}][existing_label_image_uri]" value="{{ $pair['left']['image_uri'] }}" />
                  </div>
                  <input type="file" name="question_data[options][{{ $i }}][label_image]" class="mb-2" />
                  <input type="hidden" name="question_data[options][{{ $i }}][match_type]" value="image" />
                @else
                  <input type="text" name="question_data[options][{{ $i }}][label_text]" value="{{ $pair['left']['word'] }}"
                         class="w-full p-2 border border-gray-300 rounded-lg focus:ring-gray-200" />
                  <input type="hidden" name="question_data[options][{{ $i }}][match_type]" value="text" />
                @endif
              </div>
              ...
              <div class="mb-4">
                ...
                @if (($pair['right']['match_type'] ?? 'text') === 'image')
                  <div class="mb-2">
                    ...
                    <input type="hidden" name="question_data[options][{{ $i }}][existing_value_image_uri]" value="{{ $pair['right']['image_uri'] }}" />
                  </div>
                  <input type="file" name="question_data[options][{{ $i }}][value_image]" class="mb-2" />
                  <input type="hidden" name="question_data[options][{{ $i }}][value_type]" value="image" />
                @else
                  <input type="text" name="question_data[options][{{ $i }}][value_text]" value="{{ $pair['right']['word'] }}"
                         class="w-full p-2 border border-gray-300 rounded-lg focus:ring-gray-200" />
                  <input type="hidden" name="question_data[options][{{ $i }}][value_type]" value="text" />
                @endif
              </div>
            </div>
          @endforeach
...
@if ($question->type === 'rearranging')
            <div class="mb-6">
                <label class="block mb-2 font-semibold">Words/Fragments to Rearrange</label>
                @foreach ($question->options as $index => $option)
                    <input type="text" name="question_data[options][]"
                        value="{{ old('question_data.options.' . $index, $option->value) }}"
                        required
                        class="w-full p-3 mb-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-4 focus:ring-indigo-200" />
                @endforeach
            </div>

            <div class="mb-6">
                <label class="block mb-2 font-semibold">Correct Order</label>
                <input type="text" name="question_data[correct_order]"
                    value="{{ old('question_data.correct_order', implode(' ', $question->answer['answer'] ?? [])) }}"
                    placeholder="Enter words in correct order separated by space"
                    required
                    class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-4 focus:ring-indigo-200" />
                <p class="mt-1 text-sm text-gray-500">Words should be exactly as in options, separated by spaces.</p>
            </div>
        @endif
            ...
            <div>
                ...
                @foreach ($comprehension as $i => $comp)
                    <div class="p-5 mb-8 border border-gray-300 rounded-lg bg-gray-50">
                        ...
                        <input type="text" name="question_data[comprehension][{{ $i }}][question_name]" required
                        ...
                        @foreach ($comp['blanks'] ?? [] as $bIndex => $blank)
                            <div class="mb-6">
                                <label class="block mb-1 font-semibold">Blank #{{ $bIndex + 1 }} Correct Answer</label>
                                <select name="question_data[comprehension][{{ $i }}][blanks][{{ $bIndex }}][answer]" required
                                ...
                                @foreach ($blank['options'] ?? [] as $optIndex => $option)
                                    <input type="text" name="question_data[comprehension][{{ $i }}][blanks][{{ $bIndex }}][options][]"
                ...
                <p class="text-sm text-gray-500">To add or remove questions/blanks, please recreate the question.</p>
            </div>
        @endif
